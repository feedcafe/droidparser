...
static bdaddr_t *remote_devices = NULL;
...
void bdaddr_conflict_detect(const bdaddr_t *ba)
{
	...
	/* compare with local adapter */
	for (i = 0; i < remote_devices_cnt; i++) {
		if (!bacmp(ba, &remote_devices[i])) {
			printf("\033[44;37m###############################################\033[0m\n");
			printf("\033[44;37mremote device has same mac with local adapter\033[0m\n");
			printf("\033[44;37m%2.2X:%2.2X:%2.2X:%2.2X:%2.2X:%2.2X\033[0m\n",
	...
	/* compare with each other between remote devices */
	for (i = 0; i < remote_devices_cnt; i++)
		for (j = i+1; j < remote_devices_cnt; j++)
			if (!bacmp(&remote_devices[i], &remote_devices[j])) {
				bdaddr = &remote_devices[i];
				printf("\033[44;37m###############################################\033[0m\n");
				printf("\033[44;37mthere are two remote device has same mac\033[0m\n");
				printf("\033[44;37m%2.2X:%2.2X:%2.2X:%2.2X:%2.2X:%2.2X\033[0m\n",
...
void add_remote_device(const bdaddr_t *addr)
{
	...
	if (remote_devices == NULL) {
		remote_devices = malloc(4 * sizeof(bdaddr_t));
	...
	if (remote_devices_cnt >= remote_devices_capacity) {
		...
		remote_devices = realloc(remote_devices, sizeof(bdaddr_t) *
